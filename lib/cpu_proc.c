...
static void proc_stop(cpu_context *ctx) { fprintf(stderr, "STOPPING!\n"); }
...
static void proc_di(cpu_context *ctx) { ctx->int_master_enable = false; }

static void proc_ei(cpu_context *ctx) { ctx->enabling_ime = true; }

static bool is_16_bit(reg_type rt) { return rt >= RT_AF; }
...
static void proc_ld(cpu_context *ctx) {
  ...
  if (ctx->cur_inst->mode == AM_HL_SPR) {
    u8 hflag =
        (cpu_read_reg(ctx->cur_inst->reg_2) & 0xF) + (ctx->fetch_data & 0xF) >=
        0x10;
    ...
    cpu_set_reg(ctx->cur_inst->reg_1,
                cpu_read_reg(ctx->cur_inst->reg_2) + (char)ctx->fetch_data);
...
static void proc_sub(cpu_context *ctx) {
  u16 val = cpu_read_reg(ctx->cur_inst->reg_1) - ctx->fetch_data;
  ...
  int h = ((int)cpu_read_reg(ctx->cur_inst->reg_1) & 0xF) -
          ...
          0;
  int c =
      ((int)cpu_read_reg(ctx->cur_inst->reg_1)) - ((int)ctx->fetch_data) < 0;
...
static void proc_sbc(cpu_context *ctx) {
  ...
  int h = ((int)cpu_read_reg(ctx->cur_inst->reg_1) & 0xF) -
          ...
          0;
  int c = ((int)cpu_read_reg(ctx->cur_inst->reg_1)) - ((int)ctx->fetch_data) -
              ((int)CPU_FLAG_C) <
          0;
...
static void proc_add(cpu_context *ctx) {
  u32 val = cpu_read_reg(ctx->cur_inst->reg_1) + ctx->fetch_data;
  ...
  if (ctx->cur_inst->reg_1 == RT_SP) {
    val = cpu_read_reg(ctx->cur_inst->reg_1) + (char)ctx->fetch_data;
  }

  int z = (val & 0xFF) == 0;
  int h =
      (cpu_read_reg(ctx->cur_inst->reg_1) & 0xF) + (ctx->fetch_data & 0xF) >=
      0x10;
  ...
  if (is_16bit) {
    ...
    h = (cpu_read_reg(ctx->cur_inst->reg_1) & 0xFFF) +
        ...
        0x1000;
    u32 n = ((u32)cpu_read_reg(ctx->cur_inst->reg_1)) + ((u32)ctx->fetch_data);
    c = n >= 0x10000;
  }

  if (ctx->cur_inst->reg_1 == RT_SP) {
    z = 0;
    h = (cpu_read_reg(ctx->cur_inst->reg_1) & 0xF) + (ctx->fetch_data & 0xF) >=
        0x10;
    c = (int)(cpu_read_reg(ctx->cur_inst->reg_1) & 0xFF) +
            (int)(ctx->fetch_data & 0xFF) >=
        0x100;
  }
